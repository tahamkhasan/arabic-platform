...
export default function TeacherQuizzesPage() {
  ...
  // ── مُعدَّل: themeColor ثابت (BRAND.deep) — لا اختيار شخصي بعد الآن ──
...
fetch(`/api/quizzes?${params.toString()}`, {
  ...
  // ── جديد: حذف اختبار (مسودة أو منشور) — كانت هذه الميزة غائبة
  // تماماً، فتبقى مسودات التوليد الفاشل/الملغى عالقة بلا طريقة
  // للتخلص منها ──────────────────────────────────────────────
  async function deleteQuiz(e: React.MouseEvent, quizId: string, title: string) {
    e.stopPropagation() // لا تفتح صفحة الاختبار عند الضغط على حذف
    if (!accessToken) return
    if (!confirm(`هل تريد حذف اختبار "${title}" نهائياً؟ هذا الإجراء لا يمكن التراجع عنه.`)) return
    ...
    try {
      ...
      if (!res.ok) {
        alert(data?.error || 'فشل حذف الاختبار.')
        return
      }
    ...
    <div dir="rtl" style={{ minHeight: '100vh', background: T.bg, color: T.text, fontFamily: BRAND.fontBody, paddingBottom: 90 }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(247,242,234,0.97)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${T.border}`, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => router.push('/teacher')} style={{ padding: '10px 16px', borderRadius: 12, border: 'none', background: themeColor, color: '#fff', fontWeight: 900, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>→ رجوع</button>
        ...
        <button
          ...
          style={{ padding: '10px 20px', borderRadius: 12, border: 'none', background: BRAND.gradMain, color: '#fff', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          ＋ اختبار جديد
        </button>
      </header>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {quizzes.map(q => (
              <div key={q.id} onClick={() => router.push(`/teacher/quizzes/${q.id}`)}
                style={{ padding: '16px 18px', borderRadius: 14, background: T.cardBg, border: `1.5px solid ${T.border}`, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  ...
                  <button
                    onClick={(e) => deleteQuiz(e, q.id, q.title)}
