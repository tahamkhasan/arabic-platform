...
export default function DashboardPage() {
  ...
  const bg = isDark
    ...
    : 'linear-gradient(135deg,#f0f4ff,#e8ecf8,#dde6ff)'
  ...
  async function selectSubject(s: any) {
    ...
    setUnits([]); setLessons([]); setOutput(''); setError('')
  ...
  async function selectUnit(u: any) {
    setSelUnit(u); setSelLesson(null); setLessons([]); setOutput(''); setError('')
    ...
    <div dir="rtl" style={{ minHeight: '100vh', padding: '16px', color: textMain, background: bg, fontFamily: "'Segoe UI', Tahoma, Arial, sans-serif", transition: 'all 0.3s' }}>
      ...
      {showSettings && (
        <>
          ...
          {/* القائمة */}
          <div style={{
            position: 'fixed', top: 0, left: 0, bottom: 0, width: 300, zIndex: 50,
            ...
            {/* معلومات المستخدم */}
            <div style={{ padding: '14px', borderRadius: 14, marginBottom: 20, background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)', border: `1px solid ${borderCol}` }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                {THEME_COLORS.map(c => (
                  <button key={c.value} onClick={() => saveTheme(c.value, themeMode)}
                    style={{
                      padding: '10px 6px', borderRadius: 12, border: themeColor === c.value ? `2px solid ${c.value}` : '2px solid transparent',
                      background: c.gradient, cursor: 'pointer', color: '#1a1a2e',
                      fontWeight: 700, fontSize: 11, transition: 'all 0.2s',
                      boxShadow: themeColor === c.value ? `0 4px 12px ${c.value}44` : 'none',
                    }}>
            ...
            <button onClick={handleLogout} style={{
              width: '100%', padding: '13px', borderRadius: 14, border: '1.5px solid rgba(252,129,129,0.3)',
              background: 'rgba(252,129,129,0.08)', color: '#fc8181',
              fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 16,
            }}>🚪 تسجيل الخروج</button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  <button key={mode} onClick={() => saveTheme(themeColor, mode)}
                    style={{
                      padding: '11px 14px', borderRadius: 12, border: `1.5px solid ${themeMode === mode ? themeColor : borderCol}`,
                      background: themeMode === mode ? themeColor + '15' : 'transparent',
                      color: themeMode === mode ? themeColor : textSub,
                      fontWeight: 700, fontSize: 13, cursor: 'pointer',
                      fontFamily: 'inherit', textAlign: 'right', transition: 'all 0.2s',
                    }}>
          ...
          : <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {subjects.map(s => (
              <button key={s.id} onClick={() => selectSubject(s)} style={{
                ...
                background: selSubject?.id === s.id ? themeGradient : cardBg,
                color: selSubject?.id === s.id ? '#1a1a2e' : textSub,
                boxShadow: selSubject?.id === s.id ? `0 4px 14px ${themeColor}44` : 'none',
      ...
      {selSubject && units.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {units.map(u => (
              <button key={u.id} onClick={() => selectUnit(u)} style={{
                padding: '9px 14px', borderRadius: 12, border: 'none', cursor: 'pointer',
                fontWeight: 700, fontSize: 12, transition: 'all 0.2s',
                background: selUnit?.id === u.id ? themeGradient : cardBg,
                color: selUnit?.id === u.id ? '#1a1a2e' : textSub,
      ...
      {selUnit && lessons.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {lessons.map(l => (
              <button key={l.id} onClick={() => { setSelLesson(l); setTool(null); setOutput(''); setExamMode(null) }} style={{
                padding: '12px 14px', borderRadius: 12, border: selLesson?.id === l.id ? `2px solid ${themeColor}` : `1.5px solid ${borderCol}`,
                cursor: 'pointer', fontWeight: 700, fontSize: 13, textAlign: 'right',
                background: selLesson?.id === l.id ? themeColor + '15' : cardBg,
                color: selLesson?.id === l.id ? themeColor : textSub,
          ...
          {selLesson && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
              {TOOLS.filter(t => t.id !== 'exam').map(t => (
                <button key={t.id} onClick={() => { setTool(t.id); setExamMode(null); setSelExam(null); setOutput('') }} style={{
                  padding: '12px 8px', borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: 12, textAlign: 'right',
                  border: tool === t.id ? `2px solid ${themeColor}` : '2px solid transparent',
                  background: tool === t.id ? themeColor + '15' : cardBg,
                  color: tool === t.id ? themeColor : textSub, transition: 'all 0.2s',
                }}>
            ...
            <div style={{ padding: 14, borderRadius: 14, border: `1px solid ${borderCol}`, background: cardBg }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                {[['short','📝 قصير'],['final','📊 نهائي']].map(([type, label]) => (
                  <button key={type} onClick={() => { setExamMode(type as any); setTool('exam'); setSelLesson(null); setSelExam(null); setOutput('') }} style={{
                    flex: 1, padding: '9px', borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12,
                    background: examMode === type ? themeGradient : isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)',
                    color: examMode === type ? '#1a1a2e' : textSub, transition: 'all 0.2s',
                  }}>{label}</button>
                ))}
              </div>
              {examMode && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {(examMode === 'short' ? shortExams : finalExams).map(e => (
                    <button key={e.id} onClick={() => { selectExam(e); setOutput('') }} style={{
                      padding: '9px 12px', borderRadius: 10, textAlign: 'right', cursor: 'pointer', fontWeight: 700, fontSize: 12,
                      border: selExam?.id === e.id ? `1px solid ${themeColor}` : `1px solid ${borderCol}`,
                      background: selExam?.id === e.id ? themeColor + '15' : 'transparent',
                      color: selExam?.id === e.id ? themeColor : textSub, transition: 'all 0.2s',
                    }}>
                      {e.name}
      ...
      {((tool && tool !== 'exam' && selLesson) || (tool === 'exam' && selExam)) && (
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea value={userInput} onChange={e => setUserInput(e.target.value)}
            ...
            rows={2}
            style={{ width: '100%', padding: '12px', borderRadius: 12, border: `1.5px solid ${borderCol}`, background: inputBg, color: textMain, fontSize: 13, resize: 'none', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit', lineHeight: 1.9, transition: 'all 0.3s' }} />
        </div>
      )}

      {error && <p style={{ color: '#fc8181', fontSize: 12, textAlign: 'center', marginBottom: 10, padding: '8px', borderRadius: 10, background: 'rgba(252,129,129,0.08)' }}>⚠️ {error}</p>}
      ...
      {((tool && tool !== 'exam' && selLesson) || (tool === 'exam' && selExam)) && (
        <button onClick={handleGenerate} disabled={loading} style={{
          ...
          background: loading ? (isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)') : themeGradient,
          color: loading ? textSub : '#1a1a2e', fontWeight: 900, fontSize: 15,
          boxShadow: loading ? 'none' : `0 6px 18px ${themeColor}44`,
          transition: 'all 0.3s', marginBottom: 20, fontFamily: 'inherit',
        }}>
