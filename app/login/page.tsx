...
const C = {
  ...
  blue:     '#2563EB',
  blueGlow: '0 8px 28px rgba(37,99,235,0.45)',
}
    ...
    <div dir="rtl" style={{ minHeight:'100vh', display:'flex', fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", background: C.bg }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes glow   {0%,100%{box-shadow:0 6px 24px rgba(37,99,235,0.4);}50%{box-shadow:0 10px 40px rgba(37,99,235,0.65);}}
        ...
        .field input::placeholder{color:rgba(107,80,80,0.45);}

        /* زر CTA أزرق متوهج — مميز */
        .btn-blue{
          transition:transform 0.18s,box-shadow 0.18s;
          animation:glow 3s ease-in-out infinite;
        }
        .btn-blue:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 12px 40px rgba(37,99,235,0.6)!important;}
        ...
        .btn-sec:hover{border-color:rgba(192,57,43,0.35)!important;color:${C.red}!important;background:rgba(192,57,43,0.05)!important;}
      ...
      <div className="right-side" style={{
        ...
        {/* تدرج كريمي فوق الصورة */}
        <div style={{ position:'absolute', inset:0, background:`linear-gradient(160deg, ${C.bg}F5 0%, rgba(245,240,232,0.88) 100%)` }} />
        {/* أضواء خلفية دافئة */}
        ...
        <div style={{ position:'relative', zIndex:2, maxWidth:500 }}>
          ...
          {/* شعار مِداد */}
          <div className="a1" style={{ display:'flex', alignItems:'center', gap:14, marginBottom:48 }}>
            <div style={{
            ...
            <div>
              <div style={{ fontSize:26, fontWeight:900, color: C.text, lineHeight:1 }}>مِداد</div>
              <div style={{ fontSize:13, color: C.sub, fontWeight:600, marginTop:3 }}>العربية بذكاء</div>
            </div>
          </div>
          ...
          {/* وصف قصير */}
          <p className="a3" style={{ fontSize:16, color: C.sub, lineHeight:1.8, marginBottom:14 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div className="fi feat-mini" style={{ gridColumn:'1/-1', display:'flex', alignItems:'center', gap:16, padding:'16px 18px', borderRadius:14, background:`rgba(192,57,43,0.05)`, border:`1px solid ${C.border}` }}>
              ...
              <div>
                <div style={{ fontSize:14, fontWeight:800, color: C.text, marginBottom:3 }}>شرح ذكي للدروس</div>
                <div style={{ fontSize:13, color: C.sub }}>شرح + ورقة عمل + تصحيح فوري بالذكاء الاصطناعي</div>
              </div>
            </div>
            {/* صغيرة */}
            {[
              { icon:'🎯', title:'اختبارات', sub:'8 أسئلة وتصحيح فوري' },
              { icon:'📊', title:'متابعة',   sub:'تحليلات للطرفين' },
            ].map((f,i) => (
              <div key={i} className="fi feat-mini" style={{ padding:'16px', borderRadius:14, background:`rgba(192,57,43,0.04)`, border:`1px solid ${C.border}` }}>
                ...
                <div style={{ fontSize:12, color: C.sub, lineHeight:1.6 }}>{f.sub}</div>
              </div>
            ))}
          </div>
      ...
      <div className="left-side" style={{
        ...
        {/* شريط ملون علوي بألوان مِداد */}
        <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:`linear-gradient(90deg,${C.deep},${C.red},${C.orange},${C.gold})` }} />
        {/* ضوء خلفي */}
        ...
        <div style={{ width:'100%', maxWidth:310, position:'relative', zIndex:2 }}>
          ...
          {error && (
            <div className="a1" style={{
              padding:'11px 14px', borderRadius:10, marginBottom:16, fontSize:14,
              background: error.startsWith('✅') ? 'rgba(16,185,129,0.08)' : 'rgba(192,57,43,0.08)',
              border:`1px solid ${error.startsWith('✅') ? 'rgba(16,185,129,0.3)' : 'rgba(192,57,43,0.3)'}`,
            ...
            <div className="field a3">
              ...
              <div style={{ position:'relative' }}>
                ...
                <button type="button" onClick={() => setShowPass(s => !s)}
                  style={{ position:'absolute', left:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', fontSize:15, color: C.sub, padding:0, lineHeight:1 }}>
                  {showPass ? '🙈' : '👁️'}
                </button>
              </div>
            </div>

            {/* زر الدخول — أزرق متوهج مميز */}
          ...
          <p style={{ textAlign:'center', fontSize:12, color:`${C.sub}60`, marginTop:20 }}>
            مِداد • الكويت 🇰🇼
          </p>
        </div>
      </div>
