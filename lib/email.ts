...
export async function sendRegistrationEmail(user: {
  ...
  await resend.emails.send({
    from: FROM,
    to: ADMIN_EMAIL, // مؤقتاً للاختبار — Resend يرسل فقط لإيميل الحساب
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #0d1117; color: #e2e8f0; border-radius: 16px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          <h2 style="color: #f9d423;">طلب تسجيل جديد من ${user.name} 👋</h2>
          <p style="color: #a0aec0; line-height: 1.8;">تم استلام طلب تسجيل جديد يحتاج موافقتك.</p>
          ...
          <div style="text-align: center; margin: 28px 0;">
            <a href="${APP_URL}/admin" style="background: linear-gradient(135deg,#f9d423,#ff4e50); color: #1a1a2e; padding: 14px 32px; border-radius: 12px; text-decoration: none; font-weight: 900; font-size: 16px;">
              مراجعة الطلب ←
            </a>
          </div>
...
export async function sendApprovalEmail(user: {
  ...
  await resend.emails.send({
    from: FROM,
    to: ADMIN_EMAIL, // مؤقتاً للاختبار
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #0d1117; color: #e2e8f0; border-radius: 16px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          ...
          <div style="text-align: center; margin: 28px 0;">
            <a href="${APP_URL}" style="background: linear-gradient(135deg,#f9d423,#ff4e50); color: #1a1a2e; padding: 14px 32px; border-radius: 12px; text-decoration: none; font-weight: 900; font-size: 16px;">
              دخول المنصة ←
            </a>
          </div>
          <p style="color: #4a5568; font-size: 12px; margin-top: 32px;">منصة مساعد اللغة العربية • الكويت 🇰🇼</p>
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #0d1117; color: #e2e8f0; border-radius: 16px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          ...
          <p style="color: #a0aec0;">للاستفسار أو تقديم طلب جديد، تواصل مع المدير.</p>
          <p style="color: #4a5568; font-size: 12px; margin-top: 32px;">منصة مساعد اللغة العربية • الكويت 🇰🇼</p>
        </div>
      </div>
    `,
  ...
  await resend.emails.send({
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; background: #0d1117; color: #e2e8f0; border-radius: 16px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          <p style="color: #a0aec0; line-height: 1.8;">تلقينا طلباً لإعادة تعيين كلمة مرور حساب: <strong style="color: #4facfe;">${email}</strong></p>
          <div style="text-align: center; margin: 28px 0;">
