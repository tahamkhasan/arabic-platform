...
// ══════════════════════════════════════════════════════
// ── مُعدَّل: تستدعي الآن راوتر Claude/Gemini الموحَّد بدل
// استدعاء Anthropic مباشرة. السلوك الظاهري للمستدعي (نفس
// التوقيع، نفس نوع الإرجاع) لم يتغيّر إطلاقاً — فقط الداخل. ──
// ══════════════════════════════════════════════════════
import { generate } from './ai/providers/router'
...
export async function generateWithGemini(
  ...
  if (result.fellBack) {
    console.log(`ℹ️ أداة "${systemPrompt.slice(0, 30)}..." وُلِّدت عبر Gemini الاحتياطي.`)
  }
  return result.text || 'لم تُسترجع نتيجة.'
}